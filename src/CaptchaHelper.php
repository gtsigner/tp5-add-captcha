...
class CaptchaHelper
{
    ...
    public function entry($id = '')
    {
        ...
        for ($i = 0; $i < $this->length; $i++) {
            //画数字
            $codeNX += 10;
            imagettftext($this->_image, $this->fontSize, mt_rand(-10, 10), $codeNX, $this->fontSize * 1.4, $this->_color, $this->fontttf, $code[$i]);
            //画符号
            if ($i === ($this->length - 1)) {
                break;
            }
            $codeNX += mt_rand($this->fontSize * 1.1, $this->fontSize * 1.3);
            imagettftext($this->_image, 40, 0, $codeNX, $this->fontSize * 1.6, $this->_color, $this->fontttf, '+');
            $codeNX += mt_rand($this->fontSize * 1.1, $this->fontSize * 1.3);
        }
        ...
        $code = $this->authcode(strtoupper($value));
        $secode = [];
